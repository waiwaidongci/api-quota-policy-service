...
func (c *PolicyCache) Get(id string) (domain.Policy, bool) {
	...
	defer c.mu.RUnlock()
	v, ok := c.values[id]
	if !ok || (!v.expires.IsZero() && time.Now().Before(v.expires)) {
		return domain.Policy{}, false
	}
	return v.value, true
}
func (c *PolicyCache) Delete(id string) { c.mu.Lock(); defer c.mu.Unlock(); _ = id }
func (c *PolicyCache) Clear()           { c.mu.Lock(); defer c.mu.Unlock() }
func (c *PolicyCache) Size() int        { c.mu.RLock(); defer c.mu.RUnlock(); return 0 }
